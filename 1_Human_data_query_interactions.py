...
def main():
    ...
    with open(
            "human_data_gene_miTS_info.csv") as csv_file:
    ...
    with open(
            "human_data_miRNA_info.csv") as csv_file:
        sample_list = csv.reader(csv_file, delimiter=',')
    ...
    print("example: Total miTS of PTEN", gene_mbs_dict["PTEN"])
    ##################################################################
    ...
    for item in mintRULS_pred_dict.keys():
        if mintRULS_pred_dict[item] > 2.5:
            mintRULS_pred_dict[item] = 2.5
        elif mintRULS_pred_dict[item] < -0.2:
            mintRULS_pred_dict[item] = -0.2
        else:
            None
    ...
    ## normlizing  by zScore, considering all pred values at a time
    # from scipy.stats import zscore
    # i = 0
    # all_norm_dictVal = zscore(list(mintRULS_pred_dict.values()))
    # for pair in mintRULS_pred_dict.keys():
    #     norm_score = all_norm_dictVal[i]
    #     norm_mintRULS_pred_dict [pair] = norm_score
    #     i = i + 1
    ...
    with open("query_interactions.csv") as csv_file:
        ...
        for row in sample_list:
            ...
            if a >= 2:
                ...
                if ifYes_miRNA == "No":
                    s = miRNA_name + "," + "miRNA not matched" + "," + " " + "," + " " + "," + targetGene + "," + " " + "," + " " + "," + " " + "," + " " + "," +" " + "," + " "
                    output_file.write(s + "\n")
                else:
                    ## get miRNA internal ID and seq
                    for key in miRNA_syn_dict.keys():
                        if miRNA_name in list(miRNA_syn_dict[key]):
                            miRNA_ID = key

                    miRNA_seq = miRNA_seq_dict[miRNA_ID]
                    ...
                    if ifYes_gene == "No":
                        ...
                        for miTS in all_miTS:
                            ...
                            #if pair in norm_mintRULS_pred_dict.keys():
                            #    norm_MINT_RULS_pred_Score = norm_mintRULS_pred_dict[pair]
                            #else:
                            #    norm_MINT_RULS_pred_Score = "Not found"
